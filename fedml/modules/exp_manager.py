...
class ExperimentManager():
    ...
    def log(self, update_dict, nested = False, printout=False, override=False):
        ...
        for key, value in update_dict.items(): 
            if (not key in self.results) or override:
                ...
                if nested and isinstance(value, dict):
                    for sub_key, sub_value in value.items():
                        if (not sub_key in self.results[key]):
                            self.results[key][sub_key] = [sub_value]
                        else:
    ...
    def load_from_disc(self, path, filename, verbose=False):
