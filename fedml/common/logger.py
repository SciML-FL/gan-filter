...
# Create logger
LOGGER_NAME = "fedml"
FEDML_LOGGER = logging.getLogger(LOGGER_NAME)
FEDML_LOGGER.setLevel(logging.DEBUG)
...
FEDML_LOGGER.addHandler(console_handler)
...
def configure(
    identifier: Optional[str] = None, filename: Optional[str] = None, host: Optional[str] = None
) -> None:
    ...
    if identifier: string_to_input += f"{identifier} | "

    string_to_input += "%(levelname)s %(name)s %(asctime)s "
    ...
    if filename:
        ...
        FEDML_LOGGER.addHandler(file_handler)
    ...
    if host:
        ...
        FEDML_LOGGER.addHandler(http_handler)
...
def warn_preview_feature(name: str) -> None:
    ...
    log(
        ...
        """PREVIEW FEATURE: %s
            ...
            entirely in future versions of FedML.
        """,
        name,
    )
...
def warn_deprecated_feature_with_example(
    ...
    log(
        ...
        """DEPRECATED FEATURE: %s
            ...
            new mechanism to use this feature in FedML.
        """,
        deprecation_message,
    )
    log(
